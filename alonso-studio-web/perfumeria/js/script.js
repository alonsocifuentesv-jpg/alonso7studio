  ...
  e.forEach(x => {
    if (x.isIntersecting) {
      x.target.querySelectorAll('.count').forEach(c => {
        const t = +c.dataset.target; let n = 0; const s = t / 55;
        const i = setInterval(() => { n = Math.min(n + s, t); c.textContent = Math.floor(n).toLocaleString('es-CL'); if (n >= t) clearInterval(i); }, 22);
      });
...
function ckNext(n) {
  if (n === 2) {
    ...
    for (const f of fields) {
      ...
      if (!el.value.trim()) { el.focus(); el.style.borderColor = '#ef4444'; setTimeout(() => el.style.borderColor = '', 1800); return; }
    }
...
const ASERVS = [
  { name: 'Asesoría personal', dur: 30, price: 0 },
  { name: 'Prueba de aromas', dur: 45, price: 5000 },
  { name: 'Set personalizado', dur: 60, price: 8000 }
];

const ABARBERS = [
  { name: 'Alonso Cifuentes', spec: 'Perfumes hombre & colección' },
  { name: 'Alonso Studio', spec: 'Perfumes mujer & niños' }
];

const OCC = {
  0: { 0: ['09:00','09:30','13:00','14:30'], 1: ['10:00','11:00','16:00'], 2: ['09:00','15:00'], 3: ['12:30','13:00'] },
  1: { 0: ['11:00','14:00'], 1: ['09:30','15:30','16:00'], 2: ['12:00','16:30'], 3: ['10:00','10:30'] }
};
...
function buildCal() {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    btn.innerHTML = `<div class="dn">${days[d.getDay()]}</div><div class="dd">${d.getDate()}</div><div class="dm">${months[d.getMonth()]}</div>`;
    btn.onclick = () => { asel.dayIdx = i; asel.time = ''; document.querySelectorAll('.day-btn').forEach(b => b.classList.remove('selected')); btn.classList.add('selected'); document.getElementById('btn2').disabled = true; buildSlots(i); };
    c.appendChild(btn);
  }
}
...
function buildSlots(d) {
  ...
  for (let h = 8; h < 19; h++) {
    for (let m = 0; m < 60; m += 30) {
      ...
      div.className = 'slot' + (occ.includes(label) ? ' occ' : '');
      div.textContent = occ.includes(label) ? 'Ocupado' : label;
      if (!occ.includes(label)) {
        div.onclick = () => { asel.time = label; document.querySelectorAll('.slot:not(.occ)').forEach(s => s.classList.remove('selected')); div.classList.add('selected'); document.getElementById('btn2').disabled = false; };
      }
      g.appendChild(div);
    }
  }
...
function resetAgenda() {
  asel = { barber: -1, service: -1, dayIdx: -1, time: '', pago: -1 };
  document.querySelectorAll('.barber-btn,.serv-opt,.pago-opt').forEach(el => el.classList.remove('selected'));
  document.getElementById('cardSim').style.display = 'none';
